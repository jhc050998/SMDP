...
nTrain = length(train_labels); % 60000
nTest = length(test_labels); % 10000
% 训练与测试集样本个数

global threshold tau_m tau_s Vnorm T dt Timeline groupSize scaleRate Nclass
...
groupSize = 1; scaleRate = 0.1; Nclass = 10;
...
tic;
for lr = 1: 1 % 3个不同的学习率各跑一次 length(lr_w_o)**
...
%             dropNeuron = find(rand(size(th1,1),1) < dropRate); % dropout层
%             if isempty(th1) % 随机取些神经元，取消其输出
%                 continue;
%             else
%                 th1(dropNeuron,:) = Inf; th2(dropNeuron,:) = Inf;
%             end
         ...
         % 测试过程
%         numCorrect = Aug_testing (train_pattern(:, 1:nTrain), train_labels(1:nTrain), w_h, w_o);
%         fprintf(' trainCorrect = %d, trainAccuracy = %2.4f\n',numCorrect,numCorrect/nTrain);
%         if numCorrect > maxTrain(lr) 
%             maxTrain(learnRate) = numCorrect; % 记录最大训练正确率
%         end
        ...
        if numCorrect > maxTest(lr)
            maxTest(lr) = numCorrect; % 记录最大测试正确率
            % 保存测试最大正确率时的权重
            max_w_o{lr} = w_o; max_w_h{lr} = w_h;  
        end

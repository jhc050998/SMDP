...
factor = exp( - dt / tau_m );
No = size(w,2);    % w的第二个维度的大小�?��?�输出层神经元数�?
outNum = zeros(No,1);
to1 = [];
to2 = [];
cj = [];
...
if ~isempty(psp) 
    if ndims(psp) == 3  % 有延�? 求出来是三个维度，需要改变一下顺�?
        psp = permute(psp,[1 3 2]); % permute交换psp第二第三维度
        V0 = squeeze(sum(w .* psp, 1)); % sum(a,dim); a表示矩阵；dim等于1或�??2�?1表示每一列进行求和，2表示每一行进行求和；   squeeze：去�?1维的向量�?  —�?�VO�?终为每个神经元的每时刻电压�??
    else
        V0 =  w' * psp;
    end
else
...
for k = 1: length(Timeline)
    E_k = E_k * factor;  % 神经元后�?项，每个输出随着时间衰减
    Vo(:, k) = V0(:, k) - E_k;                             
    row = find( Vo(:, k) >= theta );   % 找到大于theta的神经元
    temp_cj = zeros(No , 1); % 记录每一时刻的脉冲系�?
    while ~isempty(row)
...
%             E_k(row) = E_k(row) + theta;   %***
    ...
    temp = Inf(No, 1);
    temp(temp_cj>0) = Timeline(k); %  每个神经元中，脉冲系数大�?0，则为有输出
    cj = [cj temp_cj]; % 记录每个脉冲时刻的脉冲系�?
    to1 = [to1 temp];  % 记录每个神经元的脉冲时刻 
end
if ~isempty(to1)
    to1 = sort(to1,2);  % 对数组进行排�?
    to2 = sort(to2,2);
    to1(:, all(to1 == Inf, 1)) = [];  %（不是消除所有inf，是消除�?有都是inf的列
    to2(:, all(to2 == Inf, 1)) = [];  
    outNum = sum(cj,2);
end
